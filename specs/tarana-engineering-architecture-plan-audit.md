...
**Current next-task ranking after PR #575:**
1. **Continue 5.1 structured-logging close-out** — migrate the remaining live API `console.*` emitters in bounded domain slices; do not combine all 62 files into one PR.
...
The multi-agent generator idempotency gap is closed by PR #571, refresh-to-generator forwarding by PR #572, parser logging by PR #573, approved production dead-file cleanup by PR #574, and the first auth logging slice by PR #575. The remaining standalone diagnostics are intentionally retained.
