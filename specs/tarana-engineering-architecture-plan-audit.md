...
**Current next-task ranking after PR #589:**
1. **Migrate `refreshScheduler.ts` logging** — the cron route is now active and authenticated; convert the service's remaining title-bearing/raw-error console output without changing evaluation behavior.
2. **Decide monitor route scope** — keep, rewrite, or delete the unauthenticated stateful placeholder before adding observability.
3. **Unify `CreditService.ensureUserProfile`** — remove the fourth default-profile insert path after checking money-path behavior.
