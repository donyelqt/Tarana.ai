...
|---|---|---|---|
| [~] | 5.1 | Structured logs everywhere — itinerary-generator slices 1–7 + refresh slice 7 + food-recommendations route (PR #565) | PRs #551 (`844be3b`), #552 (`1d7b5f3`), #553 (`53ab3f0`), #554 (`e5146d0`), #555 (`de1c95d`), #556 (`5b2b40c`), #559 (`5641ae7`), #562 (`d07643e`), each merged after `verify` SUCCESS. Generator tree: 115 console calls across 9 files converted with messages preserved verbatim and structured meta attached. Refresh route (PR #562): 88 calls converted — GET/POST handlers log via `logger` + `getRequestId(request)`; module helpers log with `{ entryPoint: 'refresh' }` meta per the 5.1-R2 convention. Food route (PR #565): 29 route calls plus 4 shared error-handler calls converted to `logger`; request-owned helpers receive `getRequestId(req)`, every source carries `entryPoint: 'food-recommendations'`, and raw prompt/full-response logging was removed. Local gates: focused 3/3, full suite 639 passed / 6 skipped / 0 failed, tsc clean, lint 0 errors (pre-existing warnings only), production build green, CRLF preserved (916/916), reviewed files have zero `console.*`. Current non-test `.ts` scan (excluding `__tests__`, `*.test.ts`, `*.spec.ts`): 65 files / 578 hits; remaining food-related emitters are `robustFoodJsonParser.ts` (4), `route_legacy.ts` (5), and `test-food-recommendations-fix.ts` (7). §5.1 full close-out NOT done. |
...
**Current next-task ranking after PR #569:**
1. **Multi-agent generator idempotency** — production `USE_MULTI_AGENT=true` bypasses the normal generator claim at `itinerary-generator/route.ts:245-247`; this is now the highest remaining money-safety gap.
2. **Refresh internal idempotency forwarding** — add a stable key to the refresh-to-generator call after the multi-agent path honors keys.
3. **5.1 parser logging** — convert the 4 live `robustFoodJsonParser.ts` calls; lower risk than the two charge-path gaps.
4. **Dead-file cleanup** — `route_legacy.ts`, `src/middleware/logger.ts`, and test utilities remain zero-inbound-reference cleanup candidates; deletion still requires an explicit decision.

Explicitly deferred: 2.4 Redis rate limiting (ADR/traffic decision), 5.2 tracing (no sink), 5.3 alerting (no channel), 1.3/1.4 architecture/API migration, 4.x cross-client testing, and Phase 6/7 product work.
...
The next engineering priority remains **multi-agent generator idempotency**: production `USE_MULTI_AGENT=true` still bypasses the normal generator claim at `itinerary-generator/route.ts:245-247`. Refresh key forwarding remains dependent on that server contract. Parser logging and dead-file cleanup remain lower-risk follow-ups.