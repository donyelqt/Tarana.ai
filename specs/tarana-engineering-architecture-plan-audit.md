...
|---|---|---|---|
| [~] | 5.1 | Structured logs everywhere — itinerary-generator slices 1–7 + refresh slice 7 | PRs #551 (`844be3b`), #552 (`1d7b5f3`), #553 (`53ab3f0`), #554 (`e5146d0`), #555 (`de1c95d`), #556 (`5b2b40c`), #559 (`5641ae7`), #562 (`d07643e`), each merged after `verify` SUCCESS. Generator tree: 115 console calls across 9 files converted with messages preserved verbatim and structured meta attached. Refresh route (PR #562): 88 calls converted — GET/POST handlers log via `logger` + `getRequestId(request)`; module helpers (`transformItineraryStructure`, `enrichItineraryWithTraffic`, `regenerateItinerary`) log with `{ entryPoint: 'refresh' }` meta per the 5.1-R2 convention; one validation-failure path upgraded `info` → `error`. Zero behavior change (wire shapes, status codes, control flow untouched); refresh suite 5/5, full suite green. §5.1 full close-out NOT done: 67 non-test `.ts` files outside these trees still emit `console.*` (615 hits; 23 `.tsx` client files are outside §5.1's `grep --include=*.ts` verify). |
