...
- **3.3a lockfile hygiene — DONE (2026-09-26, this slice).** `scripts/check-lockfile-integrity.mjs` (zero-dep) asserts the committed file's structural invariants: one lockfile, default registry only, no `tarball:` URLs, every resolution entry with a sha512 integrity hash (1334/1334 today), `packageManager` pin `pnpm@9.5.0`. Wired as a blocking `Lockfile hygiene` step in BOTH CI jobs (verify after Dependency audit, e2e-smoke after install). Honest boundary: this is NOT provenance — pnpm 9.5 has no `audit signatures` subcommand and lockfile v9.0 carries zero attestation fields; integrity hashes are registry content hashes, not publisher signatures.
- **3.3b attestation/signature verification — OPEN.** Needs pnpm ≥10 (`audit signatures`) or an npm-provenance/Sigstore sink decision. Revisit trigger: pnpm major bump or §3.3 sink decision.
- Original slice text (superseded): `npm audit --audit-level=high` already planned in Phase 0.4; add `pnpm audit` signatures for provenance. Shipped as `pnpm audit --audit-level=critical --prod` (PR #491); signatures unavailable on the pinned pnpm 9.5.
- **Verify:** a PR that adds a vulnerable dependency fails CI (existing `pnpm audit` gate); a PR that adds a competing lockfile, tarball URL, registry override, or unpins the manager fails the hygiene gate.
...
- Use Playwright (`@playwright/test` in `devDependencies`, `playwright.config.ts`, CI `e2e-smoke` job on every PR).
- Cover: signup → login → generate itinerary → save → view in dashboard. The happy-path legs need live Supabase/Gemini/ledger and are NOT asserted in CI (stub env terminates them non-deterministically); the journey slice pins the deterministic boundaries instead: register validation 400s (missing fields, ToS refusal, bad email, weak password), generator 401 unauthenticated, save 401 unauthenticated + 401-before-400 precedence.
...
| [x] | 3.3a | Lockfile hygiene gate | **Done (this slice).** `scripts/check-lockfile-integrity.mjs` (zero-dep, NOT provenance): one lockfile, default registry only, no `tarball:` URLs, 1334/1334 sha512 integrity, `packageManager` pin `pnpm@9.5.0`. Blocking `Lockfile hygiene` step in BOTH CI jobs. Integrity hashes are registry content hashes, not publisher signatures — zero attestations exist in lockfile v9.0. |
| [ ] | 3.3b | Attestation/signature verification | Needs pnpm ≥10 (`audit signatures`) or npm-provenance/Sigstore sink. Revisit on pnpm major bump or §3.3 sink decision. |
...
(2m43s). Independent review: APPROVE, 2 Required nits fixed pre-merge.
...
Converted 29 direct `console.*` calls in `src/app/api/gemini/food-recommendations/route.ts` and 4 calls in `src/lib/foodRecommendationErrorHandler.ts` to the zero-dep structured logger. `getRequestId(req)` is threaded through request-owned helpers; `entryPoint: 'food-recommendations'` is explicit metadata; raw prompt and full Gemini response content are no longer logged. The no-candidate throw, credit refund path, response envelopes, and status codes are unchanged. Independent review returned APPROVE; its Required shared-handler console finding and Optional info-level visibility finding were fixed in follow-up commit `2546a63`. Verification: focused route tests 3/3; full suite 639 passed / 6 skipped / 0 failed; `tsc --noEmit` clean; `next lint --max-warnings=1000` 0 errors with pre-existing warnings; `next build` green; CRLF 916/916; reviewed files contain zero `console.*`.
