...
| [x] | 0.3 | RED metrics (zero-dep) | **Done** (PRs #533 + #534 + #535). Deliberately NOT `prom-client` (zero-dep convention per `logger.ts`/`withRetry`/flags — no new dependency, no lockfile churn). `src/lib/observability/httpMetrics.ts`: `observeHttp` accumulator + `timedHttp` wrapper + Prometheus text exposition, labels bounded to method/route(`static template`)/status_class, fixed buckets [0.05..5]. `GET /api/metrics` exposes counters + histogram (7 core + 2 route tests). 5 hottest routes instrumented: `tiers/all`, `health`, `credits/balance`, `referrals/stats`, `locations/search`. `docs/slo.md` sourcing updated to live metrics. |
