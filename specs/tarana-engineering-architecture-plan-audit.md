...
**Current next-task ranking after PR #573:**
1. **Dead-file cleanup** — `route_legacy.ts`, `src/middleware/logger.ts`, and test utilities remain zero-inbound-reference cleanup candidates; deletion still requires an explicit decision.
...
The multi-agent generator idempotency gap is closed by PR #571, refresh-to-generator forwarding by PR #572, and the 5.1 parser logging gap by PR #573. Dead-file cleanup remains the next lower-risk follow-up.
