...
| [ ] | 3.2 | SSRF protection | Partial — guard + 12 rejection tests landed (PR #606 `7e8e3e3`: `src/lib/security/ssrf.ts` `isSafeUrl`, `__tests__/ssrf.test.ts` incl. `169.254.169.254` rejection) but zero production callers (`git grep isSafeUrl origin/main -- src` → tests only); `imageService`/TomTom fetch paths unwired. |
...
**Current next-task ranking after PR #606 (verified on `origin/main` @ `b614914`):**
1. **Decide the SSRF helper's fate** — `isSafeUrl` + 12 tests landed (#606) with zero production callers; either wire into the relevant outbound image-fetch boundary or formally record no user-controlled fetch URL route and close §3.2.
2. **Expand E2E smoke into the full Phase 4.1 journey** — smoke (4 tests + CI job, #605) is the stable baseline; the audited signup → login → generate → save → dashboard path is still missing.
3. **Mobile shippability decision** — still product-scope blocked (`eas.json`, tests, CI job, local-AI stub, offline behavior); decision before code.

Closed since the prior ranking: **unify `CreditService.ensureUserProfile`** (#604 via `e9094b2` — delegates to `userService.createUserProfile`/`userProfileExists`, `src/lib/referral-system/CreditService.ts:20,33-46`) and **delete `ensureFullItinerary` dead code** (#604 via `5e938fe` — `git grep ensureFullItinerary origin/main -- src` → zero).
