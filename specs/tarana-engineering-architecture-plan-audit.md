...
**Current next-task ranking after PR #585:**
1. **Continue 5.1 structured-logging close-out** — migrate the remaining live API `console.*` emitters in bounded domain slices; the next candidate requires a fresh route-by-route scan.
2. **Unify `CreditService.ensureUserProfile`** — remove the fourth default-profile insert path after checking money-path behavior.
3. **Delete `ensureFullItinerary` dead code** — zero callers are verified; deletion remains approval-gated cleanup.
...
The multi-agent generator idempotency gap is closed by PR #571, refresh-to-generator forwarding by PR #572, parser logging by PR #573, approved production dead-file cleanup by PR #574, auth logging by PR #575, credit-diagnostics logging by PR #576, spots logging by PR #577, locations logging by PR #578, stats logging by PR #579, traffic-analysis logging by PR #581, route-calculation logging by PR #583, and referrals-debug logging by PR #585. The remaining standalone diagnostics are intentionally retained.
