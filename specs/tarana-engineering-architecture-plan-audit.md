...
**Current next-task ranking after PR #572:**
1. **5.1 parser logging** — convert the 4 live `robustFoodJsonParser.ts` calls; the charge-path idempotency gaps are closed.
2. **Dead-file cleanup** — `route_legacy.ts`, `src/middleware/logger.ts`, and test utilities remain zero-inbound-reference cleanup candidates; deletion still requires an explicit decision.
...
The multi-agent generator idempotency gap is closed by PR #571 and refresh-to-generator forwarding is closed by PR #572. Parser logging and dead-file cleanup remain lower-risk follow-ups.
