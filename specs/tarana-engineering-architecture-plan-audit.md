...
|---|---|---|---|
| [~] | 5.1 | Structured logs everywhere | Route slices through PR #587 are converted; the authenticated cron route is active. Remaining work is `src/lib/services/refreshScheduler.ts` and the stateful `routes/monitor` placeholder. Full close-out is not done. |
