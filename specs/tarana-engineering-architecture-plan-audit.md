...
**Current next-task ranking after PR #576:**
1. **Continue 5.1 structured-logging close-out** — migrate the remaining live API `console.*` emitters in bounded domain slices; the next reviewed candidate is `/api/spots`.
...
The multi-agent generator idempotency gap is closed by PR #571, refresh-to-generator forwarding by PR #572, parser logging by PR #573, approved production dead-file cleanup by PR #574, auth logging by PR #575, and credit-diagnostics logging by PR #576. The remaining standalone diagnostics are intentionally retained.
