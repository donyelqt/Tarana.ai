...
**Current next-task ranking after PR #594:**
1. **Unify `CreditService.ensureUserProfile`** — remove the fourth default-profile insert path after checking money-path behavior.
2. **Delete `ensureFullItinerary` dead code** — zero callers are verified; deletion remains approval-gated cleanup.
3. **Begin Phase 4.1 E2E coverage** — implement the audited signup → login → generate itinerary → save → dashboard path in CI.
