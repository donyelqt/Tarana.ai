...
**Current next-task ranking after PR #571:**
1. **Refresh internal idempotency forwarding** — add a stable key to the refresh-to-generator call now that the multi-agent server path honors keys.
2. **5.1 parser logging** — convert the 4 live `robustFoodJsonParser.ts` calls; lower risk than the charge-path work.
3. **Dead-file cleanup** — `route_legacy.ts`, `src/middleware/logger.ts`, and test utilities remain zero-inbound-reference cleanup candidates; deletion still requires an explicit decision.
...
The multi-agent generator idempotency gap is closed by PR #571. Refresh key forwarding is now the next charge-path task; parser logging and dead-file cleanup remain lower-risk follow-ups.
