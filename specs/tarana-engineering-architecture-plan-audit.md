...
**Current next-task ranking after PR #577:**
1. **Continue 5.1 structured-logging close-out** — migrate the remaining live API `console.*` emitters in bounded domain slices; the next candidate is `/api/locations/search`, which needs new route coverage.
...
The multi-agent generator idempotency gap is closed by PR #571, refresh-to-generator forwarding by PR #572, parser logging by PR #573, approved production dead-file cleanup by PR #574, auth logging by PR #575, credit-diagnostics logging by PR #576, and spots logging by PR #577. The remaining standalone diagnostics are intentionally retained.
