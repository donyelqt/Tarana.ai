...
- [ ] Task 1: `refund_credits` RPC migration — atomic guarded decrement (`UPDATE ... WHERE (daily - used) ...`, floor against actual consumption, not zero), `idempotency_key TEXT UNIQUE` on `credit_transactions`, `LEAST(match_count,50)` on the embeddings FN in the same migration.
