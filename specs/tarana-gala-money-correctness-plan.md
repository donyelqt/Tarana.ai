...
- [ ] Task 9: Delete `TemporalOptimizer`, renormalize weights (no-op by construction — prove with ranking snapshot test); extract `weatherTypeMapper` (kills ×4), `promptBuilder` (kills 9-arg), `durationParser`; move root `FIX_*.sql` into `supabase/migrations/`; lazy `getSupabaseAdmin()` + 32KB body cap. **CORRECTION 2026-09-19 (Osmani review + verified):** `.strict()` does NOT exist on the installed `@supabase/postgrest-js@2.104.1` (runtime probe: `typeof builder.strict === 'undefined'`; `maxAffected()` also absent). Drop `.strict()` from scope; the real gaps are lazy `getSupabaseAdmin()` and the 32KB body cap (route `req.json()` is unbounded at `route.ts:275`).
