...
### Build status (merged to main, 2026-09-15 — PR #477)

7.1 SQLite store (`db/index.ts` + `.web.ts` stub, profiles/trips/meals, v2 migration), 7.2 SavedTrips→SQLite, 7.4 screens (ProfileCreate/LinkAccount/AuthEntry rebind/Settings/nav), Spots parity (top-3 + derived badges + Maps CTA), TripDetail, SavedCafes + CafeDetail, Eats (20-cafe vendored catalog), Plan form (Phase-2-gated generate), Explore (list-based routes), Home hub, data seam (`data/index.ts`), weather-shape fix. Gates green: `tsc` exit 0, Metro `-p web` complete, banned imports zero. Outstanding: device run on new paths, then sliced PRs. 7.3 bundled-cache, 7.5 hygiene, 7.6 proxy hardening: not started.

**Shipped 2026-09-15 (PR #477, commits `afaeb4c` + `5ac3ac8`):** web saved-meals import into SQLite (`importWebMeals` twin of `importWebTrips`, `upsertImportedMeal` native + web stub, single browser round-trip, degrade-to-trips-only) + saved-dish snapshot on local save + `Your saved dishes` in CafeDetail. Outstanding: device run on the new import path.
...
| Saved trips list `/saved-trips` | DONE (SQLite, §7.2) + search parity 2026-09-15 (`a126554`): header-band search, title/id/tag filter, filtered vs create/import empty states | — |
| Trip detail `/saved-trips/[id]` | SHIPPED 2026-09-15 (`a126554`): Morning-split Day N + date headers, `selectedInterests` line, canonical `TrafficBadge`, Reviews/Maps actions into `ActivitySheet` (native PlaceDetail: About/Location/system-Maps/Coming-Soon reviews; Share/Save dropped as dead; no Smart Refresh — session-cookie server-side, no mobile token path) | Reads the SQLite `payload` already stored by import. Web refresh (`GET/POST …/refresh`) deferred to post-Phase-4 local re-run. |
| Saved meals `/saved-meals` + detail | SHIPPED 2026-09-15 (PR #477): `importWebMeals` twin import + `upsertImportedMeal` + `Your saved dishes` in CafeDetail + dish snapshot on local save | `getSavedMealById` filters `.eq('id')` with no `user_id` (`supabaseMeals.ts:31-36`) — mobile never repeats this; trips pattern (profile-scoped everywhere) is the rule. |
...
Screens import ONLY `../data` (+ `../config` for link URLs, navigator props). Verified by grep: zero screen imports of `../db`, `../supabase`, `../auth`. Current bindings — profiles/trips/meals local SQLite; spots/weather session-free HTTP with boundary validation (untrusted responses); `importWebTrips` + `importWebMeals` one-way copies (JWT single-use, never persisted; meals degrade-to-trips-only); `signOut` drops profile pointer + token. Swapping any domain local↔HTTP later = one function body, zero screen rewrites.

### Screen build status (merged to main, 2026-09-15)

Done: Landing, AuthEntry shell, ProfileCreate, LinkAccount (trips + meals twin import), Home (spots-only hub), SavedTrips (SQLite + thumbs + search parity `a126554`), TripDetail (`a126554`: day groups + `ActivitySheet`), Spots (top-3 parity + badges + Maps CTA), SavedCafes + CafeDetail (PR #477: web import + saved dishes), Eats (vendored catalog + local suggest), Plan form (Phase-2-gated generate), Explore (list-based routes + TomTom embed), Settings, MainTabs + ActionSheet, grouped-canvas + gradient-pill polish, canonical GradientCTA (auth/app variants from verified web stops). Next: device run on the new import path, then sliced PRs. 7.3 bundled-cache, 7.5 hygiene, 7.6 hardening: not started.
