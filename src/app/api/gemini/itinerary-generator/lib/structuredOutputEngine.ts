...
export class StructuredOutputEngine {
  ...
  static async generateStructuredItinerary(
    ...
    logger.info(`🏗️ STRUCTURED ENGINE: Starting generation for request ${requestId}`, { entryPoint: 'structuredOutputEngine', requestId });
    ...
    try {
      ...
      try {
        ...
        logger.info(`✅ STRUCTURED ENGINE: Raw JSON received in ${Date.now() - startTime}ms`, { entryPoint: 'structuredOutputEngine' });

        // Validate and clean the structured data
        const validatedItinerary = this.validateAndCleanStructure(rawData, requestId);

        logger.info(`🎯 STRUCTURED ENGINE: Successfully generated valid itinerary in ${Date.now() - startTime}ms`, { entryPoint: 'structuredOutputEngine' });
        ...
        logger.error(`❌ STRUCTURED ENGINE: All attempts failed for request ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error: error?.logMessage ?? error });
        return this.createFallbackItinerary(requestId);
      }
    } catch (error: any) {
      logger.error(`💥 STRUCTURED ENGINE: Critical error for request ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error });
      return this.createFallbackItinerary(requestId);
    }
  }
  ...
  private static parseStructuredJson(text: string, requestId: string): any {
    try {
      ...
      logger.warn(`🔄 STRUCTURED ENGINE: Direct parse failed for ${requestId}, attempting recovery...`, { entryPoint: 'structuredOutputEngine', requestId });
    }
    ...
    if (syntaxCheck.cleanedJson) {
      try {
        ...
        logger.warn(`🔄 STRUCTURED ENGINE: Syntax cleaning failed for ${requestId}, attempting aggressive fix...`, { entryPoint: 'structuredOutputEngine', requestId });
      }
    }
    ...
    try {
      ...
      logger.warn(`🔄 STRUCTURED ENGINE: Custom fixer failed for ${requestId}, invoking jsonrepair...`, { entryPoint: 'structuredOutputEngine', requestId });
    }
    ...
    try {
      ...
      logger.error(`❌ STRUCTURED ENGINE: jsonrepair failed for ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error: error instanceof Error ? error.message : error });
  ...
  private static decodeNestedJson(value: unknown, requestId: string): any {
    if (typeof value === 'string') {
      ...
      if ((trimmed.startsWith('{') && trimmed.endsWith('}')) || (trimmed.startsWith('[') && trimmed.endsWith(']'))) {
        try {
          logger.warn(`🔁 STRUCTURED ENGINE: Detected stringified JSON for ${requestId}, parsing again.`, { entryPoint: 'structuredOutputEngine', requestId });
          const reparsed = JSON.parse(trimmed);
          return this.decodeNestedJson(reparsed, requestId);
        } catch (error) {
          logger.warn(`⚠️ STRUCTURED ENGINE: Nested JSON parse failed for ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error: error instanceof Error ? error.message : error });
          return trimmed;
        }
      }
    }
...
DO NOT return explanatory text, markdown, or anything other than pure JSON.`;
  ...
  private static validateAndCleanStructure(
    ...
    try {
      ...
      logger.info(`✅ STRUCTURED ENGINE: Schema validation passed for ${requestId}`, { entryPoint: 'structuredOutputEngine', requestId });
      return validated;
      
    } catch (zodError: any) {
      logger.warn(`🔧 STRUCTURED ENGINE: Schema validation failed, attempting fixes for ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error: zodError.message });
      ...
      try {
        ...
        logger.info(`✅ STRUCTURED ENGINE: Schema validation passed after fixes for ${requestId}`, { entryPoint: 'structuredOutputEngine', requestId });
        return revalidated;
      } catch (secondError: any) {
        logger.error(`❌ STRUCTURED ENGINE: Unable to fix schema issues for ${requestId}:`, { entryPoint: 'structuredOutputEngine', requestId, error: secondError.message });
        return this.createFallbackItinerary(requestId);
      }
    }
  }
  ...
  private static createFallbackItinerary(requestId: string): StructuredItinerary {
    logger.info(`🆘 STRUCTURED ENGINE: Creating fallback itinerary for ${requestId}`, { entryPoint: 'structuredOutputEngine', requestId });
...
export class StructuredOutputMonitor {
  ...
  static async performHealthCheck(): Promise<void> {
    ...
    logger.info(`🏥 STRUCTURED ENGINE: Health check - ${health.status}`, { entryPoint: 'structuredOutputEngine', status: health.status });
  }
}
