...
import { rankSpots, spotPool, spotsSubtitle, toSpotCard } from "../utils";
...
const SuggestedSpots = () => {
  ...
  const spots = useMemo(() => {
    ...
    return { ranked, cards: cards.slice(0, 3) };
  }, []);

  if (spots.cards.length === 0) return null;

  return (
    <div className="mb-8">
      <div className="flex justify-between items-center mb-6 px-1">
        <h2 className="font-medium text-xl text-gray-900">Suggested Spots</h2>
        <p className="text-sm text-gray-500">{spotsSubtitle(spots.ranked)}</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {spots.cards.map((spot) => (
