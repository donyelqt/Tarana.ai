...
const PlaceDetail = ({ place }: PlaceDetailProps) => {
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              {place.images.slice(1, 4).map((image, index) => (
                <div key={index} className="relative h-24 rounded-2xl overflow-hidden border border-gray-200/40 shadow-sm transition-all duration-300 hover:shadow-md hover:scale-[1.02]">
                  <Image 
                    src={image} 
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button variant="outline" className="border-gray-300 hover:bg-gray-50 transition-all duration-300 hover:border-gray-400 hover:-translate-y-0.5 rounded-xl shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Tabs */}
      <div className="flex border-b border-gray-200/60 mb-6 bg-white/50 backdrop-blur-sm rounded-t-2xl overflow-hidden">
        <button
          className={`px-6 py-3 font-semibold text-sm transition-all duration-300 ${activeTab === 'description' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50/50'}`}
          onClick={() => setActiveTab('description')}
        >
          Description
        </button>
        <button
          className={`px-6 py-3 font-semibold text-sm transition-all duration-300 ${activeTab === 'map' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50/50'}`}
          onClick={() => setActiveTab('map')}
        >
          View on Map
        </button>
        <button
          className={`px-6 py-3 font-semibold text-sm transition-all duration-300 ${activeTab === 'reviews' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50/50'}`}
